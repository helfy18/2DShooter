...
public class Enemy : MonoBehaviour
{
    ...
    {
        AddToScore();
        IncrementEnemiesDefeated(isBoss);
    }
    ...
    private void AddToScore()
    {
        if (GameManager.instance != null && !GameManager.instance.gameIsOver)
        {
            ...
            int afterScore = GameManager.score;
            if ((afterScore/1000) > (score / 1000))
            {
                addLife();
            }
        }
    ...
    {
        GameObject player = GameManager.PlayerGet();
        Health health = player.GetComponent<Health>();
        if ((health.currentLives + 1) <= health.maximumLives)
        {
            health.currentLives++;
            player.GetComponent<AudioSource>().pitch = 1;
            if (health.extraLifeEffect != null)
            {
                Instantiate(health.extraLifeEffect, transform.position, transform.rotation, null);
            }
        }
    ...
    /// void (no return)
    /// </summary>
    private void IncrementEnemiesDefeated(bool isBoss)
    {
        if (GameManager.instance != null && !GameManager.instance.gameIsOver)
        {
            GameManager.instance.IncrementEnemiesDefeated(isBoss);
        }       
    }
