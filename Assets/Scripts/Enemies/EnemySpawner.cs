...
public class EnemySpawner : MonoBehaviour
{
    ...
    [Tooltip("If the spawner only spawns when the player is close")]
    public bool spawnIfCloseOnly = false;
    [Tooltip("How close does the player have to be for the spawner to spawn")]
    ...
    [Tooltip("Does the spawner spawn more frequently when score is over 5000")]
    public bool increaseFrequency = false;
    [Tooltip("The time delay between spawning enemies")]
    public float spawnDelay = 2.5f;
    [Tooltip("The spawn delay when at a high score")]
    public float highSpawnDelay = 2.0f;
    ...
    {
        if (GameManager.score > 5000 && increaseFrequency)
        {
            spawnDelay = highSpawnDelay;
        }
        // If it is time for an enemy to be spawned
        if (Time.timeSinceLevelLoad > lastSpawnTime + spawnDelay && (currentlySpawned < maxSpawn || spawnInfinite) && (!spawnIfCloseOnly || (target.position - transform.position).magnitude < spawnRange))
        {
