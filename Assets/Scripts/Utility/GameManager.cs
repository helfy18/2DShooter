...
public class GameManager : MonoBehaviour
{
    ...
    //this boss isn't referenced in the code but is to track the boss of the level
    [Tooltip("The boss to end the level (if applicable)")]
    ...
    public static GameObject PlayerGet()
    {
    ...
    private void FigureOutHowManyEnemiesExist()
    {
        ...
        if (gameIsWinnable)
        {
            if (boss != null)
            {
                Debug.Log("There is a boss to defeat to complete the level, so the level is winnable");
            }
            else if (numberOfInfiniteSpawners > 0)
            {
    ...
    /// </summary>
    public void IncrementEnemiesDefeated(bool isBoss)
    {
        enemiesDefeated++;
        if ((enemiesDefeated >= enemiesToDefeat || isBoss) && gameIsWinnable)
        {
            LevelCleared();
        }
    }
