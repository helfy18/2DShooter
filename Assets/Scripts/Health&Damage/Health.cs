...
public class Health : MonoBehaviour
{
    ...
    [Tooltip("Whether to respawn at start or at place of death")]
    ...
    [Tooltip("The speed to set the pitch to when at 1 life")]
    public float musicSpeed = 1.15f;
    ...
    void Respawn()
    {
        timeToBecomeDamagableAgain = Time.time + 4;
        if (respawnAtStart)
        {
    ...
    {
        currentLives -= 1;
        if (currentLives == 1 && GetComponent<AudioSource>() != null)
        {
            GetComponent<AudioSource>().pitch = GetComponent<AudioSource>().pitch * musicSpeed;
        }
        if (currentLives > 0)
        {
            Respawn();
